...
pub fn run(file_path: String) {
  ...
  let part1 =
    list.zip(left, right)
    |> list.fold(0, fn(acc, v) { int.absolute_value(v.0 - v.1) + acc })

  let inc = fn(opt) { option.map(opt, fn(i) { i + 1 }) |> option.unwrap(or: 1) }

  let counts =
    list.fold(right, dict.new(), fn(acc, r) { dict.upsert(acc, r, inc) })
  let part2 =
    list.fold(left, 0, fn(acc, left_value) {
      left_value
      |> dict.get(counts, _)
...
fn process_lines(
  ...
  case lines {
    ...
    [line, ..lines] -> {
      let assert Ok([l, r]) =
        regexp.split(pattern, line) |> list.map(int.parse) |> result.all
